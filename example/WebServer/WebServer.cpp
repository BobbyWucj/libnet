...
WebServer::WebServer(EventLoop* loop, const InetAddress& listenAddr, const std::string& root)
...
void WebServer::onHttp(const HttpRequest& request, HttpResponse* response)
{
    ...
    if (path == "/") {
        ...
        response->setBody("<html><head><title>WebServer</title></head>"
            "<body><h1>Hello</h1>Now is " + nowFormatedString +
            "</body></html>");
        return;
    ...
    void* mmap_ret = ::mmap(NULL, file_stat.st_size, PROT_READ, MAP_PRIVATE, file_fd, 0);
    ::close(file_fd);
...
void WebServer::onError(HttpResponse* response, const HttpResponse::HttpStatusCode statusCode) {
    response->setStatusCode(statusCode);
    string body;

    body += "<html><title>哎~出错了!</title>";
    body += "<body bgcolor=\"ffffff\">";
    body += std::to_string(statusCode) + HttpResponse::statusTitleMap[statusCode];
