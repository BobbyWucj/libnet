...
TcpMainReactor::TcpMainReactor(EventLoop* loop, const InetAddress& local, const Nanoseconds heartbeat)
    : TcpReactor(loop, local, heartbeat),
      threadPool_()
{
    acceptor_->setNewConnectionCallback(std::bind(&TcpMainReactor::newConnection, this, _1, _2, _3));
}
...
void TcpMainReactor::newConnection(int connfd, const InetAddress& local, const InetAddress& peer) {
...
void TcpMainReactor::closeConnection(const TcpConnectionPtr& connPtr) {
    loop_->runInLoop(std::bind(&TcpMainReactor::closeConnectionInLoop, this, connPtr));
}
...
void TcpMainReactor::closeConnectionInLoop(const TcpConnectionPtr &connPtr) {
    ...
    ioLoop->queueInLoop(std::bind(&TcpConnection::connectionDestroyed, connPtr));
}
